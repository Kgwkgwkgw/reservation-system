define("myreservation", ["Handlebars"], function(Handlebars) {
	...
	var myreservationModelA;
	var fnCache;
	function initVariable(myreservationModel, objReservationType) {
		...
		myreservationModelA = myreservationModel;
		myreservationModelA.on("listArrive", function(){
			console.log("fwef");
		});
		myreservationModelA.on("countArrive", function(){
			console.log("fwef2");
		})
	...
	function addEventHandling() {
	    $btnAll.on("click", fnCache=function(e) {
          toggleFilterBtn.call(this, e);
		  removeMyReservationList();
	      myreservationModelA.reqReservationList();
	      myreservationModelA.reqReservationCount();
		  console.log(fnCache);
		  console.dir(fnCache);
	    });

	    $btnSchedule.on("click", fnCache=function(e) {
	      toggleFilterBtn.call(this, e);
		  removeMyReservationList();
	      myreservationModelA.reqReservationList(objType.ASKING);
	      myreservationModelA.reqReservationList(objType.CONFIRM);
	      myreservationModelA.reqReservationCount();
	    });

	    $btnCompletion.on("click", fnCache=function(e) {
	      toggleFilterBtn.call(this, e);
		  removeMyReservationList();
	      myreservationModelA.reqReservationList(objType.COMPLETION);
	      myreservationModelA.reqReservationCount();
	    });

	    $btnCancellationAndRefund.on("click", fnCache=function(e) {
	      toggleFilterBtn.call(this, e);
		  removeMyReservationList();
	      myreservationModelA.reqReservationList(objType.CANCELLATION);
	      myreservationModelA.reqReservationList(objType.REFUND);
	      myreservationModelA.reqReservationCount();
	    });

	    $myReservationList.on("click", btnReservationCancel, showCancellationPopup);
	    $myReservationList.on("click", btnRemoveCancellation, myreservationModelA.reqRemoveCancellation);
	...
	function init(myReservationModel, objReservationType) {
		initVariable(myReservationModel, objReservationType);
		addEventHandling();
